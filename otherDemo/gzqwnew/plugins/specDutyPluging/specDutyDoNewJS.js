...
$(function() {
	vm = new Vue({
		el: '#mainDiv',
		data: {
			specReportInfo: $.deepClone(specReportInfo),
			headquarterList: new Array(),
			workGroupList: new Array(),
			peopleList: new Array(),
			unitList: new Array(),
			classesList: new Array(),
			mapinfoList: new Array(),
		...
		methods: {
			...
			selectZHBUnit: function(headquarterId) {
				...
				selectingUnitModel.unitLevel = "none";
				var leftwidth = ($(document).width() - 980) / 2;
				$("#reportIframe").html("");
				$('#reportWin').dialog({
					title: "成员单位选择",
			...
			doSaveReport: function() {
				...
				// ajax保存
				$.ajax({
					url: BASEPATH + "/SpecDuty/doSaveNewSpecDuty.do",
					type: "POST",
					async: true,
		...
		mounted: function() {
			this.$nextTick(function() {
				vm.specReportInfo.watchBegin = $("#watchBeginTime").val()
				vm.specReportInfo.watchEnd = $("#watchEndTime").val()
				vm.specReportInfo.reportDeadline = $("#reportDeadlineTime").val()
				$("#watchBeginTime").datetimebox({
					width: '100%',
					required: true,
					showSeconds: false,
					editable: false,
					onChange: function(val) {
						vm.specReportInfo.watchBegin = val;
						if(vm.workGroupList.length > 0) {
							$.messager.confirm('确认', '您修改了专项勤务开始时间，是否同时修改所有工作组的工作开始时间?', function(flag) {
								if(flag) {
									for(var i = 0; i < vm.workGroupList.length; i++) {
										vm.workGroupList[i].watchBegin = val;
									}
									vm.doEditGZZ(vm.editingGZZ)
								}
							});
						}
					}
				})
				$("#watchEndTime").datetimebox({
					width: '100%',
					required: true,
					showSeconds: false,
					editable: false,
					onChange: function(val) {
						vm.specReportInfo.watchEnd = val;
						if(vm.workGroupList.length > 0) {
							$.messager.confirm('确认', '您修改了专项勤务结束时间，是否同时修改所有工作组的工作结束时间?', function(flag) {
								if(flag) {
									for(var i = 0; i < vm.workGroupList.length; i++) {
										vm.workGroupList[i].watchEnd = val;
									}
									vm.doEditGZZ(vm.editingGZZ)
								}
							});
				...
				$("#reportDeadlineTime").datetimebox({
					width: '100%',
					required: true,
					showSeconds: false,
					editable: false,
					onChange: function(val) {
						vm.specReportInfo.reportDeadline = val;
					}
				})
			});
